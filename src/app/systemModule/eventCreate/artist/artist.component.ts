...
export class ArtistComponent extends BaseComponent implements OnInit {
  ...
  deleteArtistCard(card:AccountGetModel){
    ...
    this.main.eventService.ArtistDeleteOwner
      (
        {
          account_id: this.main.CurrentAccount.id,
          id: card.id,
        ...
        subscribe((res)=>{
          ...
          if(err.status === 422){
            if(err.json()['errors']==='Invalid date')
                  this.onError.emit("Artist NOT deleted! Invalid date");
            else
              this.onError.emit(err.json()['errors']?err.json()['errors']:"Limit of artists was reached!");
          }
          else
            this.onError.emit("Artist NOT deleted!");
        });
